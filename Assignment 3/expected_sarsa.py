...
def expected_sarsa(stochastic, kingmove, num_episodes, seed, epsilon, alpha):
	...
	for iter in range(num_episodes):
		...
		while(True):
			...
			expected_nxt_state = 0

			for i in range(num_actions):
				weight = epsilon * 1.0 / (num_actions * 1.0)
				if(nxt_arm == i):
					weight += (1.0 - epsilon)
				expected_nxt_state += weight * q_table[nxt_state][i]

			#this should be changed for policies
			q_table[curr_state][arm] += alpha * (rew + expected_nxt_state - q_table[curr_state][arm])
